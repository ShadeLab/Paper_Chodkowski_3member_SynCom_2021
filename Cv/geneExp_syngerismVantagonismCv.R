...
allSigGenes <- c(Fullgenes,CvBtgenes,CvPsgenes)
...
sigGenesFullOnly <- readRDS("/mnt/research/ShadeLab/Chodkowski/JGI_SynCom/RNAseq/Chromobacterium/final_analyses_Rev/diffExp/output/sigGenesFullOnly_Cv_kallisto.rds")

#######Synergistic gene expression#######

lfc_pairwiseCombined_Int <- 2^(lfc_CvBtInt_comparedtoMonoInt.fc) + 2^(lfc_CvPsInt_comparedtoMonoInt.fc)
lfc_pairwiseCombined_25 <- 2^(lfc_CvBt25_comparedtoMono25.fc) + 2^(lfc_CvPs25_comparedtoMono25.fc)
lfc_pairwiseCombined_30 <- 2^(lfc_CvBt30_comparedtoMono30.fc) + 2^(lfc_CvPs30_comparedtoMono30.fc)
lfc_pairwiseCombined_35 <- 2^(lfc_CvBt35_comparedtoMono35.fc) + 2^(lfc_CvPs35_comparedtoMono35.fc)
lfc_pairwiseCombined_40 <- 2^(lfc_CvBt40_comparedtoMono40.fc) + 2^(lfc_CvPs40_comparedtoMono40.fc)
lfc_pairwiseCombined_45 <- 2^(lfc_CvBt45_comparedtoMono45.fc) + 2^(lfc_CvPs45_comparedtoMono45.fc)
...
write.csv(synergyGenes,"synergyGenes_Cv_kallisto.csv")
...
#Create side by side for filtering low Full community lfc values.
lfc_Full_antagInt <- as.data.frame(cbind(lfc_3memInt_comparedtoMonoInt.fc,lfc_CvBtInt_comparedtoMonoInt.fc,lfc_CvPsInt_comparedtoMonoInt.fc))
...
lfc_Full_antag25 <- as.data.frame(cbind(lfc_3mem25_comparedtoMono25.fc,lfc_CvBt25_comparedtoMono25.fc,lfc_CvPs25_comparedtoMono25.fc))
...
lfc_Full_antag30 <- as.data.frame(cbind(lfc_3mem30_comparedtoMono30.fc,lfc_CvBt30_comparedtoMono30.fc,lfc_CvPs30_comparedtoMono30.fc))
...
lfc_Full_antag35 <- as.data.frame(cbind(lfc_3mem35_comparedtoMono35.fc,lfc_CvBt35_comparedtoMono35.fc,lfc_CvPs35_comparedtoMono35.fc))
...
lfc_Full_antag40 <- as.data.frame(cbind(lfc_3mem40_comparedtoMono40.fc,lfc_CvBt40_comparedtoMono40.fc,lfc_CvPs40_comparedtoMono40.fc))
...
lfc_Full_antag45 <- as.data.frame(cbind(lfc_3mem45_comparedtoMono45.fc,lfc_CvBt45_comparedtoMono45.fc,lfc_CvPs45_comparedtoMono45.fc))
...
write.csv(antagGenes,"antagonismGenes_Cv_kallisto.csv")
...
FinalGeneTrends_f <- data.frame(condition=rep(c("Non-additive Up", "Non-additive Down (CvPsBt)","Non-additive Down (CvBt)","Non-additive Down (CvPs)"), each=6),
...
FinalGeneTrends_f$condition <- factor(FinalGeneTrends_f$condition,levels = c('Non-additive Up','Non-additive Down (CvPsBt)','Non-additive Down (CvBt)','Non-additive Down (CvPs)'),ordered = TRUE)
...
geom_bar(stat="identity", color="black", position='dodge') + ylim(0,70) +
        ...
        scale_fill_manual(values=c("black", "gray","brown2","burlywood4"))+
...
ggsave("Cv_synAntag_trends.eps",plot=p2,device="eps",width=8,height=8, units="cm",dpi=300)

ggsave("CvgenesSynvsAntag_kallisto.tif",plot=p,device="tiff",width=15, units="cm",dpi=600)
ggsave("CvgenesSynvsAntagCat_kallisto.tif",plot=p2,device="tiff",width=15, units="cm",dpi=600)
