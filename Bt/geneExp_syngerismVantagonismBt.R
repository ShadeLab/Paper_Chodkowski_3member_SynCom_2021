...
allSigGenes <- c(Fullgenes,BtCvgenes,BtPsgenes)
...
sigGenesFullOnly <- readRDS("/mnt/research/ShadeLab/Chodkowski/JGI_SynCom/RNAseq/Burkholderia/final_analyses_Rev/diffExp/output/sigGenesFullOnly_Bt_kallisto.rds")

#######Synergistic gene expression#######

lfc_pairwiseCombined_Int <- 2^(lfc_BtCvInt_comparedtoMonoInt.fc) + 2^(lfc_BtPsInt_comparedtoMonoInt.fc)
lfc_pairwiseCombined_25 <- 2^(lfc_BtCv25_comparedtoMono25.fc) + 2^(lfc_BtPs25_comparedtoMono25.fc)
lfc_pairwiseCombined_30 <- 2^(lfc_BtCv30_comparedtoMono30.fc) + 2^(lfc_BtPs30_comparedtoMono30.fc)
lfc_pairwiseCombined_35 <- 2^(lfc_BtCv35_comparedtoMono35.fc) + 2^(lfc_BtPs35_comparedtoMono35.fc)
lfc_pairwiseCombined_40 <- 2^(lfc_BtCv40_comparedtoMono40.fc) + 2^(lfc_BtPs40_comparedtoMono40.fc)
lfc_pairwiseCombined_45 <- 2^(lfc_BtCv45_comparedtoMono45.fc) + 2^(lfc_BtPs45_comparedtoMono45.fc)
...
#Filter out low FC values from dataframe
...
#Filter FC >1.2 comparing Full community to additive pairwise communities
...
#genes_syn <- c(as.character(lfc_Full_synergyInt_finalGenes$Locus.Tag),as.character(lfc_Full_synergy25_finalGenes$Locus.Tag),
#           as.character(lfc_Full_synergy30_finalGenes$Locus.Tag),as.character(lfc_Full_synergy35_finalGenes$Locus.Tag),
#           as.character(lfc_Full_synergy40_finalGenes$Locus.Tag),as.character(lfc_Full_synergy45_finalGenes$Locus.Tag))
...
write.csv(synergyGenes,"synergyGenes_Bt_kallisto.csv")
...
#Create side by side for filtering low Full community lfc values.
lfc_Full_antagInt <- as.data.frame(cbind(lfc_3memInt_comparedtoMonoInt.fc,lfc_BtCvInt_comparedtoMonoInt.fc,lfc_BtPsInt_comparedtoMonoInt.fc))
...
lfc_Full_antag25 <- as.data.frame(cbind(lfc_3mem25_comparedtoMono25.fc,lfc_BtCv25_comparedtoMono25.fc,lfc_BtPs25_comparedtoMono25.fc))
...
lfc_Full_antag30 <- as.data.frame(cbind(lfc_3mem30_comparedtoMono30.fc,lfc_BtCv30_comparedtoMono30.fc,lfc_BtPs30_comparedtoMono30.fc))
...
lfc_Full_antag35 <- as.data.frame(cbind(lfc_3mem35_comparedtoMono35.fc,lfc_BtCv35_comparedtoMono35.fc,lfc_BtPs35_comparedtoMono35.fc))
...
lfc_Full_antag40 <- as.data.frame(cbind(lfc_3mem40_comparedtoMono40.fc,lfc_BtCv40_comparedtoMono40.fc,lfc_BtPs40_comparedtoMono40.fc))
...
lfc_Full_antag45 <- as.data.frame(cbind(lfc_3mem45_comparedtoMono45.fc,lfc_BtCv45_comparedtoMono45.fc,lfc_BtPs45_comparedtoMono45.fc))
...
#Put all locus tags together
#genes_antag <- c(as.character(lfc_Full_antagInt_finalGenes$Locus.Tag),as.character(lfc_Full_antag25_finalGenes$Locus.Tag),
#           as.character(lfc_Full_antag30_finalGenes$Locus.Tag),as.character(lfc_Full_antag35_finalGenes$Locus.Tag),
#           as.character(lfc_Full_antag40_finalGenes$Locus.Tag),as.character(lfc_Full_antag45_finalGenes$Locus.Tag))
...
write.csv(antagGenes,"antagonismGenes_Bt_kallisto.csv")
...
FinalGeneTrends_f <- data.frame(condition=rep(c("Non-additive Up", "Non-additive Down (BtCvPs)","Non-additive Down (BtCv)","Non-additive Down (BtPs)"), each=6),
...
FinalGeneTrends_f$condition <- factor(FinalGeneTrends_f$condition,levels = c('Non-additive Up','Non-additive Down (BtCvPs)','Non-additive Down (BtCv)','Non-additive Down (BtPs)'),ordered = TRUE)
...
geom_bar(stat="identity", color="black", position='dodge') + ylim(0,70) +
        ...
        scale_fill_manual(values=c("black", "gray","brown2","darkgoldenrod2"))+
...
ggsave("Bt_synAntag_trends.eps",plot=p2,device="eps",width=8,height=8, units="cm",dpi=300)
...
ggsave("BtgenesSynvsAntag_kallisto.tif",plot=p,device="tiff",width=15, units="cm",dpi=600)
ggsave("BtgenesSynvsAntagCat_kallisto.tif",plot=p2,device="tiff",width=15, units="cm",dpi=600)
